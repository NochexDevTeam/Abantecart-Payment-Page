...
class ModelExtensionNochex extends Model {
	public function getMethod($address) {
		...
		if ($this->config->get('nochex_status')) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zones_to_locations WHERE location_id = '" . (int)$this->config->get('nochex_location_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

			if (!$this->config->get('nochex_location_id')) {
				$status = TRUE;
